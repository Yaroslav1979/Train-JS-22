...
class CredentialsProcessor extends AuthProcessor {
  validate(username, passkey) {
    if (username === "admin" && passkey === "admin123") {
      console.log("Аутентифікація успішна за обліковими даними");
      return true;
    } else {
      return super.validate(username, passkey);
    }
...
class ProcessorBuilder {
  constructor() {
    ...
    create() {
      return {
        validate: (username, passkey) => {          
            return this.firstProcessor.handler.validate(username, passkey);         
        }
      };
    }
